...
class BridgeGame:
    ...
    def __init__(
        self, seed: int = None, dealer: int = 0, vulnerability: dict[str, bool] = None
    ):
    ...
    def get_legal_bids(self, position: int) -> list[str]:
        ...
        for bid_record in self.bids:
            ...
            if bid == "DOUBLE":
                ...
                if level > highest_level or (
                    level == highest_level
                    and suit_order.index(suit) > highest_suit_index
                ):
                    highest_level = level
    ...
    def _finalize_contract(self):
        contract_bids = [
            b for b in self.bids if b["bid"] not in ["PASS", "DOUBLE", "REDOUBLE"]
        ]
