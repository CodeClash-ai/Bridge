...
def visualize_game(
    ...
    if not tricks_only:
        ...
        if bids:
            print_bidding_phase(
                bids, pause_callback=replay.pause if not auto_play else None
            )
            if not bidding_only:
                replay.pause()
    ...
    if not bidding_only:
        ...
        if tricks:
            print_playing_phase(
                tricks, pause_callback=replay.pause if not auto_play else None
            )
...
Examples:
  ...
  %(prog)s logs/game.json --tricks-only      # Show only tricks
    ...
    parser.add_argument("log_file", help="Path to the game log JSON file")

    parser.add_argument(
        "--auto", "-a", action="store_true", help="Auto-play mode (no manual pauses)"
    )
    ...
    parser.add_argument(
        "--bidding-only", "-b", action="store_true", help="Show only the bidding phase"
    )
    ...
    if args.bidding_only and args.tricks_only:
        print(
            "Error: Cannot specify both --bidding-only and --tricks-only",
            file=sys.stderr,
        )
        return 1
